...
static int try_str_to_int(const char* string) {
    ...
    if (*endptr != '\0') {
        exit_with_error("Missing Value", string);
    }
    return (int) result;
}
...
char* ap_get_binary_name(ArgParser* parser) {
    return parser->binary_name;
}
...
static void ap_handle_equals_opt(ArgParser* parser, const char* prefix, const char* arg, ArgStream* stream) {
    ...
    *strchr(array, '=') = '\0';
    ...
    if (option->type == OPT_FLAG) {
        free(array);
        exit_with_error("flag %s%s does not accept a value", prefix, name);
    }

    if (strlen(value) == 0) {
        free(array);
        exit_with_error("missing value for %s%s", prefix, name);
    }
...
static void ap_handle_long_opt(ArgParser* parser, const char* arg, ArgStream* stream) {
    ...
    if (map_get(parser->option_map, arg, (void**)&option)) {
        ...
        exit_with_error("missing value for --%s", arg);
    }
...
static void ap_handle_short_opt(ArgParser* parser, const char* arg, ArgStream* stream) {
    for (size_t i = 0; i < strlen(arg); i++) {
        ...
        if (strlen(arg) > 1) {
            //exit_with_error("missing value for '%c'", arg[i], arg);
            if (!option_try_set(option, (char*)(arg + i + 1))) {
                ap_set_memory_error_flag(parser);
            }
            break;
        }
...
bool ap_parse(ArgParser* parser, int argc, char** argv) {
    ...
    char *binary_name;(binary_name = strrchr(argv[0], '/')) ? ++binary_name : (binary_name = argv[0]);
    parser->binary_name = str_dup(binary_name);
    if (!parser->binary_name) {
        ap_set_memory_error_flag(parser);
    }
